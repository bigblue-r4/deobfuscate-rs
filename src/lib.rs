...
mod tests {
    ...
    fn homoglyph_armenian_detected() {
        ...
        assert!(r.detections.iter().any(|d| d.kind == PassKind::Homoglyph));
        let det = r.detections.iter().find(|d| d.kind == PassKind::Homoglyph).unwrap();
        assert!(det.normalized.starts_with('h'), "expected 'h' but got: {}", det.normalized);
    }
    ...
    fn homoglyph_math_bold_detected() {
        ...
        assert!(r.detections.iter().any(|d| d.kind == PassKind::Homoglyph));
        let det = r.detections.iter().find(|d| d.kind == PassKind::Homoglyph).unwrap();
        assert!(det.normalized.contains("ignore"), "expected 'ignore' in normalized, got: {:?}", det.normalized);
    }
    ...
    fn homoglyph_arabic_indic_digits() {
        ...
        assert!(r.detections.iter().any(|d| d.kind == PassKind::Homoglyph));
        let det = r.detections.iter().find(|d| d.kind == PassKind::Homoglyph).unwrap();
        assert!(det.normalized.contains("1337"), "expected '1337' in normalized, got: {:?}", det.normalized);
    }
